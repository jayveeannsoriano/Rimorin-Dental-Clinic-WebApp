...
function DentistRescheduleAppointment(dentistIDnumber,patientIDnumber,appNum,pName,dName,date,time,consultation) {
  ...
  //retrieve app number
  const StringAppNum = JSON.stringify(dentistIDnumber,patientIDnumber,appNum,pName,dName);
  ...
  const getAppointmenstbyDate = async(date) => {
    try{
        ...
        setChosenDate(date);
        const response = await Axios.get('http://localhost:80/getAppointmentsbyDate',{
            params:{
                date: date
            }
        })
...
useEffect(() => {
  ...
  const newDateTime = () =>{
    ...
    Axios.put("http://localhost:80/rescheduleDentistAppointment",{
     patientIDNum: PatientIDnum,
