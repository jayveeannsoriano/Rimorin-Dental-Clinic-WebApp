...
const FollowUp = (patientIDnumber,appNum,dName,dentistIDnumber,apptUUID) => {

  const StringAppNum = JSON.stringify(patientIDnumber,appNum,dName,dentistIDnumber,apptUUID);
  ...
  const apptUUIDstring = JSON.stringify(ConvertStringApp.apptUUID).replace(/"/g, "");

  return (
    <>
      <Button className="followUp-button" variant="primary" href={FollowUpAppointmentRoute+"?patientIDValue="+PatientIDNumber.substring(3)+"&patientAppNum="+PatientPrevNum.substring(1)+"&dentistName="+dentistName+"&dentistIDnumber="+dentistIDnum.substring(3)+"&apptUUID="+apptUUIDstring}>
