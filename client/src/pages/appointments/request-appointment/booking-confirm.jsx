...
const BookingConfirm = ({nextStep, prevStep,handleDateChange, values}) => {
    ...
    //insert data
    Axios.post("http://localhost:80/insertAppointment", {patientIDnumber: patientIDnumber, userNameApp: userNameApp, startDate: date,formattedDate:formattedDate, consulInput: values.consultation, getTime:time, recep:userInfo['email']})
    Axios.post("http://localhost:80/sendSMS", {phone: userInfo['mobile'] ,message:"Hi "+userInfo['fname']+"! This is from Rimorin Dental Clinic notifying you of your requested Appointment at "+date+" "+time+" due to '" + values.consultation + "'. See you there!"})
